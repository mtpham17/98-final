...
cut_avg10 <- cutree(hclust_centroid, k = 10)

centroid_dend_obj <- as.dendrogram(hclust_centroid, h = 10, leaflab = "none")
labels(centroid_dend_obj) <- rep(NA, nrow(by_country))
test <- color_branches(centroid_dend_obj, k = 10)
plot(test)
...
countries_h <- list()
for (i in 1:10) {
  countries_h[[i]] <- names_h[names_h$cluster10 == i, ]$Country
}
kable(countries_h, "latex")

countries_k <- list()
for (i in 1:10) {
  countries_k[[i]] <- names_h[names_h$cluster10 == i, ]$Country
}
kable(countries_k, "latex")
```
