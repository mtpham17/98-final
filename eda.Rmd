...
for (i in 1:10) {
  med = median(master_df_h[master_df_h$cluster10 == i, ]$literacy)
  iqr = IQR(master_df_h[master_df_h$cluster10 == i, ]$literacy)
  master_df_h[master_df_h$cluster10 == i, ]$outlier_literacy <- 
    (master_df_h[master_df_h$cluster10 == i, ]$literacy > med + 1.5 * iqr) |
    (master_df_h[master_df_h$cluster10 == i, ]$literacy < med - 1.5 * iqr)
}
...
# do this manually
ggplot(master_df_h, aes(x = cluster10, y = literacy, fill = cluster10)) +
...
ggplot(master_df_k, aes(x = cluster10, y = literacy, fill = cluster10)) +
  ...
  geom_text_repel(aes(group = cluster10, 
            ...
            position = position_dodge(width=0.75),
            hjust = "left", size = 3) + 
  ggtitle("Country Literacy by K-Means Cluster") + xlab("Policy Cluster") + theme(legend.position = "none")
dev.off()
```
...
ggplot(master_df_k, aes(x = cluster10, y = Fertility, fill = cluster10)) +
  ...
  geom_text_repel(aes(group = cluster10, 
                label = ifelse(outlier_fertility,
                  ...
                  no = '')), 
            position = position_dodge(width=0.75),
            hjust = "left", size = 3) + ggtitle("Country Fertility by K-Means Cluster") + xlab("Policy Cluster") + 
...
g <- autoplot(policy.pca, data = master_df_h, colour = 'cluster10')
...
# summary(policy.pca)
jpeg(file="pca_k.jpg")
g <- autoplot(policy.pca, data = master_df_k, colour = 'cluster10')
