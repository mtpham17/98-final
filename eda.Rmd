...
kcluster_by_country$cluster3 <- cluster.results.3$cluster
kcluster_by_country$cluster6 <- cluster.results.6$cluster
```
...
master_df <- merge(by_country, gdp, by.x = "ISO3", by.y = "Code")
master_df <- merge(master_df, population, by.x = "ISO3", by.y = "Code")
master_df <- subset(master_df, select = -c(X.x, X.y, Name.x, Name.y))
