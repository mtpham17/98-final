...
```{r}
ggplot(master_df_k[!is.na(master_df_k$Expectancy), ], 
       aes(x = cluster10, y = log(Expectancy), fill = cluster10)) +
  geom_boxplot(alpha = 0.3) +
  geom_point(aes(color = cluster10, group = cluster10), position = position_dodge(width=0.75)) +
  geom_text(aes(group = cluster10, 
                label = ifelse(test = log(Expectancy) > median(log(Expectancy)) 
                               + 1.5*IQR(log(Expectancy)) | log(Expectancy) < 
                                 median(log(Expectancy)) - 1.5*IQR(log(Expectancy)), 
                  yes = ISO3,
                  no = '')), 
            position = position_dodge(width=0.75),
            hjust = "left", size = 3)
...
# kableExtra
for (i in 1:10) {
  print(master_df_k[master_df_k$cluster10 == i, ]$ISO3)
}
```
...
```{r}
aggregate(cbind(VISA_BAN_NONE, VISA_BAN_SPECIFIC, VISA_BAN_ALL,
                HISTORY_BAN_CLEANED,
                CITIZEN_LIST_CLEANED, POLICY_LENGTH, POLICY_TYPE_NON, 
                POLICY_TYPE_COMPLETE, POLICY_TYPE_PARTIAL,
                AIR, LAND, 
